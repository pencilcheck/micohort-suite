...
import type { Page, Browser } from 'https://deno.land/x/puppeteer@16.2.0/mod.ts';
import { mapLimit } from 'https://cdn.jsdelivr.net/npm/async-es@3.2.4/+esm';
//import select from '@gizt/selector';
//import select from 'https://github.com/gizt/selector/raw/main/src/index.main.ts'
...
export async function initPage(): Promise<[Page, Browser]> {
  ...
  if (env.NODE_ENV === 'development') {
    ...
    browser = await puppeteer.connect({
      browserWSEndpoint: 'wss://chrome.browserless.io?token=796a1cbc-e5fd-4fb2-93fe-d82740588e71'
    });
  }
...
export async function findProfilesV2(page: Page, person: MicpaPerson): Promise<PrepDocType[]> {
  ...
  if (firstName && lastName) {
    ...
    await page.waitForSelector(SELECTORV2.PROFILE_SEARCH_LINKS);
